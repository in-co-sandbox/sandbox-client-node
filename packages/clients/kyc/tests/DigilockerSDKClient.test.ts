...
import { DigilockerSDKClient } from '../src/kyc/digilocker-sdk/client/DigilockerSDKClient';
import { CreateSessionRequest } from '../src/kyc/digilocker-sdk/schemas/request/CreateSessionRequest';

describe('DigilockerSDKClient', () => {
    // Test constants
    const ENTITY = 'in.co.sandbox.kyc.digilocker.sdk.session.request';
    const FLOW = 'signup';
    const DOC_TYPES: ('driving_license' | 'pan' | 'aadhaar')[] = ['driving_license'];
    const SESSIONID = 'reprehenderit dolore ipsum veniam';
    ...
    let client: DigilockerSDKClient;

    beforeEach(() => {
        const credentials = new ApiUserCredentials(API_KEY, API_SECRET);
        client = new DigilockerSDKClient(credentials);
    });
